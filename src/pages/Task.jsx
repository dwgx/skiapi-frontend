...
export default function Task() {
  ...
  const endpoint = isAdmin ? '/api/task/' : '/api/task/self';
              ...
              {loading ? <TableSkeleton rows={5} columns={6} /> :
                data.length === 0 ? (
                  <TableRow><TableCell colSpan={6}>
                    <EmptyState icon={TaskAlt} title={t('暂无任务日志')} description={t('提交任务后将在此显示记录')} />
                  </TableCell></TableRow>
                ) : data.map((item, i) => (
                  <TableRow key={item.id || i} hover>
                    <TableCell sx={{ fontSize: '0.8rem' }}>{timestamp2string(item.submit_time / 1000)}</TableCell>
                    <TableCell sx={{ fontSize: '0.8rem' }}>{item.task_id || '-'}</TableCell>
                    <TableCell><Chip label={item.platform || '-'} size="small" variant="outlined" /></TableCell>
